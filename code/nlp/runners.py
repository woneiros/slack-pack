...
if __name__ == "__main__":
    ...
    for channel in channels:
        for p in xrange(1, 8, 1):
            ...
            classified_window = classifier.classify_stream(msg_stream, low_threshold=.4, high_threshold=.7, low_step=.05, high_step=.02, max_messages=500, verbose=False)
            ...
            for t, topic in enumerate(classified_window):  # one(?) per topic
                if len(topic) >= 5:
                    viz_topics += 1
                    print 'topic {} for {} duration {} day(s) has length {}'.format(t, channel, p, len(topic))
                    viz_path = IMG_FOLDER + 'cloud_topic_{}_{}_{}_{}.png'.format(channel, 'day', p, viz_topics)
